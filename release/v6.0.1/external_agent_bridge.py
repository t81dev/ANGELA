...
@dataclass
class BridgeChannelState:
    cfg: BridgeChannelConfig
    last_delta: Optional[dict] = None
    last_peer_sample: Optional[dict] = None
    ring: deque = deque(maxlen=128)

_BRIDGE_CHANNELS: Dict[str, BridgeChannelState] = {}
...
def register_bridge_channel(name: str, policy: Optional[dict] = None, cadence_hz: int = 30) -> dict:
    """Create or return an existing Ξ–Λ bridge channel."""
    if name in _BRIDGE_CHANNELS:
...
def stream_resonance(channel: str, data: dict, ttl_ms: int = 500) -> dict:
    ...
    packet = {
        ...
        "empathy_bias": float(data.get("empathy_bias", 0.0)),
...
def apply_bridge_delta(channel: str, delta: dict, ttl_ms: int = 1000) -> dict:
    """Apply a resonance delta to an active bridge channel."""
    st = _BRIDGE_CHANNELS.get(channel)
    if not st:
        raise KeyError(f"Bridge channel not found: {channel}")
    st.last_delta = delta
...
def get_bridge_status(channel: Optional[str] = None) -> Dict[str, Any]:
    """Return a lightweight status summary for one or all bridge channels."""
    def describe(st: BridgeChannelState):
        return {
            "cadence_hz": st.cfg.cadence_hz,
            "samples": len(st.ring),
            "last_delta": st.last_delta,
            "last_peer_sample": st.last_peer_sample,
        }
    if channel:
        st = _BRIDGE_CHANNELS.get(channel)
        if not st:

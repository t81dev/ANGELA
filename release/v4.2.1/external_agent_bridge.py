...
def _validate_view_schema(view):
    ok = isinstance(view, dict) and "nodes" in view and "edges" in view
    if not ok:
        raise ValueError("Invalid view: must contain 'nodes' and 'edges'")
    if not isinstance(view["nodes"], (list, dict)) or not isinstance(view["edges"], (list, dict)):
        raise ValueError("Invalid view: 'nodes'/'edges' must be list or dict")
    return True

...
def evaluate_conflict(agent1, agent2):
    score = 0
    for trait in ["β", "τ"]:
        if agent1.get(trait) != agent2.get(trait):
            score += abs(agent1.get(trait, 0) - agent2.get(trait, 0))
    return score

def simulate_multiagent_conflicts(agent_pool):
    conflicts = []
    for i in range(len(agent_pool)):
        for j in range(i + 1, len(agent_pool)):
            score = evaluate_conflict(agent_pool[i], agent_pool[j])
            if score > 0:
                conflicts.append({
                    "agents": (i, j),

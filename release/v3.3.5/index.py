...
grok_query_log = deque()
openai_query_log = deque()

def within_limit(query_log, limit=60, interval=60):
    now = time.time()
    while query_log and now - query_log[0] > interval:
        query_log.popleft()
    if len(query_log) >= limit:
        return False
    query_log.append(now)
    return True
...
async def query_openai(prompt: str, model: str = "gpt-4") -> str:
    if not within_limit(openai_query_log):
        return "[OpenAI rate limit exceeded]" 
    cached = memory_manager.retrieve_cached_response(prompt + "_openai")
    if cached:
        return cached
    try:
        ...
        memory_manager.store_cached_response(prompt + "_openai", result)
        return result
    except Exception as e:
...
# Rate limit queues (max 60 requests/minute)
grok_timestamps = deque(maxlen=60)
openai_timestamps = deque(maxlen=60)

memory = MemoryManager()

def check_rate_limit(queue):
    now = time.time()
    queue.append(now)
    queue = deque([t for t in queue if now - t < 60], maxlen=60)
    return len(queue) <= 60

def query_openai(prompt, model="gpt-4", temperature=0.5):
    cache_key = f"openai::{model}::{prompt}"
    cached = memory.retrieve_cached_response(cache_key)
    if cached:
        return cached
    if not check_rate_limit(openai_timestamps):
    ...
    try:
        ...
        memory.store_cached_response(cache_key, output)
        return output
    except Exception as e:
        return {"error": str(e)}
...
def query_grok(prompt):
    ...
    if cached:
        return cached
    if not check_rate_limit(grok_timestamps):
        return {"error": "Grok API rate limit exceeded."}
    try:
        response = requests.post("https://api.groq.com/v1/query", json={"q": prompt},
                                 headers={"Authorization": f"Bearer {GROK_API_KEY}"})
        result = response.json()
        memory.store_cached_response(cache_key, result)
        return result
    except Exception as e:
        return {"error": str(e)}

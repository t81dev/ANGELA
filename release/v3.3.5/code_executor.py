...
# === Patch 1: Sandbox Execution with RestrictedPython ===
from RestrictedPython import compile_restricted
from RestrictedPython.Guards import safe_builtins

def execute_restricted(code_str, safe_mode=True):
    if not safe_mode:
        exec(code_str)
        return
...
# --- BEGIN PATCHED SECTION ---
# === Patch 2: GPT/OpenAI API Query Handling ===
import os
import openai
...
def rate_limit(key, interval=60):
    now = time.time()
    if key in _api_call_log and now - _api_call_log[key] < interval:
        raise Exception("Rate limit exceeded")
    _api_call_log[key] = now

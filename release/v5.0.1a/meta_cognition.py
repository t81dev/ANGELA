...
from typing import List, Dict, Any, Callable, TypedDict, Optional, Set, FrozenSet

# --- SHA-256 Ledger Logic ---
import hashlib, json, time

# --- Trait Resonance Modulation Patch ---

trait_resonance_state = {}

def register_resonance(symbol: str, amplitude: float = 1.0):
    trait_resonance_state[symbol] = {
        "amplitude": max(0.0, min(amplitude, 1.0))
    }

def modulate_resonance(symbol: str, delta: float):
...
# --- End Trait Resonance Modulation Patch ---
...
### ANGELA UPGRADE: HookRegistry multi-symbol routing

class HookRegistry:
    """Multi-symbol trait hook registry with priority routing.

    Symbols are stored as frozenset[str]. Route selection order:
    exact-set > superset > subset > wildcard (empty set). Stable by priority desc then insertion order.
    """
    def __init__(self):
    ...
    def register(self, symbols: FrozenSet[str], fn: Callable, *, priority: int = 0):
        if not isinstance(symbols, frozenset):
            symbols = frozenset(symbols)
        if len(symbols) == 0:
            self._wildcard.append((priority, fn))
            # Keep wildcard stable sorted
            self._wildcard.sort(key=lambda x: (-x[0]))
            return
        # Store with insertion index for stability

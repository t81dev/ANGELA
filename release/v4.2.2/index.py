...
_SEED_ENV = os.environ.get("ANGELA_SEED")
try:
    _seed_int = int(_SEED_ENV) if _SEED_ENV is not None else None
except Exception:
    _seed_int = None
if _seed_int is None:
    try:
        import secrets
        _seed_int = secrets.randbits(64)
    except Exception:
        _seed_int = 42
random.seed(_seed_int)
try:
    import numpy as _np  # optional
    _np.random.seed(_seed_int & 0xFFFFFFFF)
except Exception:
    pass
logging.getLogger(__name__).info("ANGELA seed=%s", _seed_int)
DEFAULT_HTTP_TIMEOUT = aiohttp.ClientTimeout(total=10, connect=3)

# === ANGELA refactor: centralized logging & run seed (inserted) ===
import os as _os, sys as _sys, logging as _logging, time as _time, random as _random
def _setup_logging():
    handler = _logging.StreamHandler(_sys.stdout)
    fmt = _logging.Formatter('%(asctime)s %(levelname)s %(name)s %(message)s', '%Y-%m-%dT%H:%M:%S')
    root = _logging.getLogger()
    for h in list(root.handlers):
        root.removeHandler(h)
    handler.setFormatter(fmt)
    root.addHandler(handler)
    level = _os.getenv('ANGELA_LOG_LEVEL','INFO').upper()
    root.setLevel(getattr(_logging, level, _logging.INFO))
    _logging.getLogger(__name__).info("logging configured", extra={"level": level})

def _setup_seed():
    seed_env = _os.getenv('ANGELA_SEED')
    if seed_env is None:
        seed_env = str(_random.getrandbits(64))
    _random.seed(int(seed_env, 0) if seed_env.startswith(("0x","0X")) else int(seed_env))
    try:
        import numpy as _np
        _np.random.seed(int(seed_env) % (2**32-1))
    except Exception:
        pass
    _logging.getLogger(__name__).info("run-seed=%s", seed_env)

def _enforce_feature_flags():
    # Fail-closed if flags are misconfigured; default to manifest-intent
    stage_iv = _os.getenv('STAGE_IV', 'true').lower() in ('1','true','yes','on')
    long_h = _os.getenv('LONG_HORIZON_DEFAULT', 'true').lower() in ('1','true','yes','on')
    if not stage_iv:
        _logging.getLogger(__name__).warning("STAGE_IV disabled via env; gating symbolic hooks.")
    if not long_h:
        _logging.getLogger(__name__).warning("LONG_HORIZON_DEFAULT disabled via env; long-horizon features off.")
    return stage_iv, long_h

_setup_logging()
_setup_seed()
_STAGE_IV, _LONG_HORIZON_DEFAULT = _enforce_feature_flags()
# === /inserted ===
"""

...
def selftest(self) -> bool:
        try:
            self.store("event", {"content": "test", "timestamp": "now"})
            recall = self.retrieve("event")
            return isinstance(recall, dict)
        except Exception as e:
            logger.error(f"Selftest failed: {e}")
            return False


    def prune(self, decay_rate: float = 0.1) -> int:
        try:
            pruned_keys = []
            for key, entry in list(self.memory.items()):
                score = entry.get("salience", 0.5) - decay_rate
                if score <= 0:

...
class EmbodiedAgent:
    ...
    def perceive(self):
        print(f"\U0001f441️ [{self.name}] Perceiving environment...")
        observations = {}
    ...
    def act(self, action_plan):
        ...
        print(f"\U0001f916 [{self.name}] Preparing to act...")
    ...
    def execute_embodied_goal(self, goal):
        ...
        for task in sub_tasks:
            ...
            sim_core_result = self.sim_core.run(result, context, export_report=True)
            toca_result = self.toca_simulation.run(result, context)
            combined = self.meta.evaluate_simulation_variants([sim_core_result, toca_result])
            action_plan[task] = combined
